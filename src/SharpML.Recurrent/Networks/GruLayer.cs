...
namespace SharpML.Recurrent.Networks
{
    ...
    public class GruLayer : ILayer
    {
        ...
        public GruLayer(int inputDimension, int outputDimension, Random rng)
        {
            ...
            _hmix = Matrix.Random(outputDimension, inputDimension, 1 / Math.Sqrt(inputDimension), rng);
            _hHmix = Matrix.Random(outputDimension, outputDimension, 1 / Math.Sqrt(outputDimension), rng);
            _bmix = new Matrix(outputDimension);
            _hnew = Matrix.Random(outputDimension, inputDimension, 1 / Math.Sqrt(inputDimension), rng);
            _hHnew = Matrix.Random(outputDimension, outputDimension, 1 / Math.Sqrt(outputDimension), rng);
            _bnew = new Matrix(outputDimension);
            _hreset = Matrix.Random(outputDimension, inputDimension, 1 / Math.Sqrt(inputDimension), rng);
            _hHreset = Matrix.Random(outputDimension, outputDimension, 1 / Math.Sqrt(outputDimension), rng);
