...
namespace SharpML.Recurrent.Networks
{
    ...
    public class RnnLayer : ILayer
    {
        ...
        public RnnLayer(int inputDimension, int outputDimension, INonlinearity hiddenUnit, Random rng)
        {
            this._inputDimension = inputDimension;
            this._outputDimension = outputDimension;
            this._f = hiddenUnit;
            _w = Matrix.Random(outputDimension, inputDimension + outputDimension, 1 / Math.Sqrt(inputDimension + outputDimension), rng);
