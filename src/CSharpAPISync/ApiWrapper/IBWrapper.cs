...
namespace CSharpAPISync.ApiWrapper
{
    internal class IBWrapper : EWrapper
    {
        ...
        private Dictionary<int, CSharpAPISync.Models.Order> PlacedOrders = new Dictionary<int, CSharpAPISync.Models.Order>();
        ...
        public void Connect()
        {
            ...
            if (!AccountNames.Contains(AccountName))
                throw new ConnectionException($"Given account {AccountName} not found among these accounts found: {String.Join(", ", AccountNames)}");
        }
        ...
        public void PlaceOrder(int orderId, CSharpAPISync.Models.Order order, bool waitTillFinishes)
        {
            ...
            if (contractDetails.LiquidHours != null)
                foreach (string dateStr in contractDetails.LiquidHours.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries))
                {
                    ...
                    if (dateStr.Length != 18)
                        throw new FormatException("Unknown liquid hours datetime format.");

                    // Parse, example: 20170515:0930-1600
                    int year = int.Parse(dateStr.Substring(0, 4), CultureInfo.InvariantCulture);
                    int month = int.Parse(dateStr.Substring(4, 2), CultureInfo.InvariantCulture);
                    int day = int.Parse(dateStr.Substring(6, 2), CultureInfo.InvariantCulture);
                    int hourFrom = int.Parse(dateStr.Substring(9, 2), CultureInfo.InvariantCulture);
                    int minutesFrom = int.Parse(dateStr.Substring(11, 2), CultureInfo.InvariantCulture);
                    int hourTo = int.Parse(dateStr.Substring(14, 2), CultureInfo.InvariantCulture);
                    int minutesTo = int.Parse(dateStr.Substring(16, 2), CultureInfo.InvariantCulture);
        ...
        public void updateAccountValue(string key, string value, string currency, string accountName)
        {
            ...
            if (property == null)
            {
                _____________________________________________________________________________Logger.Warning($"Unexisting property of {nameof(Account)} called: {key}");
                return;
            }

            // Parse value
            object parsedValue = Utilities.ParseValue(property.PropertyType, value);
