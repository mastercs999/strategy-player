...
using CSharpAPISync.ApiWrapper;
    ...
    public class ApiClient
    {
        ...
        public void ConnectSafe()
        {
            try
            {
                ...
                _____________________________________________________________________________Logger.Info("Checking internet connection by pinging google.com...");
                using (Ping ping = new Ping())
                {
                    PingReply reply = ping.Send("google.com");
                    if (reply.Status != IPStatus.Success)
                        throw new ConnectionException($"Pinging google.com was unsuccessful. We received status '{reply.Status}'");
                }
                _____________________________________________________________________________Logger.Info("Ping worked, internet connection should be fine");
            ...
            {
                throw new ConnectionException("Something went wrong during safe connection. See inner exception!", ex);
            }
        }
